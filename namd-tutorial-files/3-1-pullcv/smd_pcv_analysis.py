...
Can be used for
1. Force (acc) [in direction on pull] vs time
2. SMD atom(s) COM displacement [in direction on pull] vs time
3. Fixed-SMD distance (end-to-end distance) [in direction on pull] vs time
...
def analyze_smd_pcv(namd_log_files: [str, list],
    ...
    At each timestep, the function calculates
        1. smd_com_force: the mag of force acting on the COM of SMD atom(s)
            [in the direction of applied pull, if specified]

        2. smd_com_displacement: the distance of the COM of SMD atom(s) from its initial position
            [in the direction of applied pull, if specified]

        3. [if position of the COM of fixed atom(s) is given]
            smd_fixed_dist: the distance between COM's of SMD and fixed atom(s)
...
if __name__ == "__main__":
    ...
    namd_log_files = ["ubq_wb2/ubq_wb_pcv.log"]
    pull_dir_vec = (0.27545452208402327, 0.5308679514381627, 0.8014387215497583)  # or None
    fixed_atoms_com_pos = (27.661, 22.594, 3.752)  # or None

    out_file = "analysis/ubq2_wb_pcv.csv"
    out_file_delimiter = "\t"
