...
#################################################################################################
### Script to calculate Energies from simulation trajectories (STATIC SELECTIONS ONLY)		   ##
#----------------------------------------------------------------------------------------------##
# This script is ONLY FOR STATIC atom selections (which do not change in frames)
# For DYNAMIC SELECTIONS, use namd_energy.dynamic.sh
#----------------------------------------------------------------------------------------------##
...
# It does not load dcd files into memory, and namd_processes trajectory frame-by-frame.
#
## LIMITATIONS -----------
...
# 5. run with "./namd_energy.static.tcl"
# 	OR
...
set tmp_dir     	"";		# "" OR "/tmp/namd_energy.static" for RAM disk
...
#------------------------------------------
# Temp files, Cleanup and Exit Handlers
#------------------------------------------
...
proc cleanup { } {
	global debug_mode namd_exit_code;
	global namd_temp_files_prefix;

	if { $debug_mode == 0 && $namd_exit_code == 0 } {
		puts "\n--------------------------------------"
...
puts $namdconf "coordinates \t\t [file normalize ${interaction_pdb_filename}];"
puts $namdconf "\noutputname \t\t [file normalize ${namd_temp_files_prefix}];"
...
puts $namdconf "pairInteractionFile \t\t [file normalize ${interaction_pdb_filename}];"
...
foreach dcd_file $dcd_files {
	puts $namdconf "coorfile open dcd [file normalize ${dcd_file}];"
...
puts "---------------------"
puts " => Executing: [file normalize ${namd_sh_filename}] $namdconf_filename > $namd_log_filename"
puts "=================================================================================================\n"

set namd_exit_code [catch {exec [file normalize ${namd_sh_filename}] $namdconf_filename > $namd_log_filename}];

puts "\n----------------------------"
puts " => NAMD Exit Code: $namd_exit_code";
puts "----------------------------\n"
...
# cleanup is handled by exit handler
...
if { $namd_exit_code != 0 } {
	puts "---------------------------"
	puts "=> NAMD-RUN FAILED (exit code ${namd_exit_code})"
