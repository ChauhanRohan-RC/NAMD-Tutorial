...
# resname TIP3 and around 3.5 protein
SELECTION1 = os.getenv("NAMD_ENERGY_SELECTION1", "protein")
SELECTION2 = os.getenv("NAMD_ENERGY_SELECTION2", "resname TIP3 and around 3 protein")
...
for i in range(base_system.getNumParticles()): pair_system.addParticle(base_system.getParticleMass(i))
if PERIODIC: pair_system.setDefaultPeriodicBoxVectors(*base_system.getDefaultPeriodicBoxVectors())
...
print("-------------------------------------")
log_info(f"TOTAL ATOM COUNT: {u_init.atoms.n_atoms}")
...
elec_base = f"({138.935456 / DIELECTRIC} * charge1 * charge2 / r * {S_elec})"
...
# --- FEATURE 7: 1-4 Exclusions & Re-injection ---
...
elec_14_force = mm.CustomBondForce(f"{138.935456 / DIELECTRIC} * q_14 / r")
...
for i in range(nb_base.getNumExceptions()):
    ...
    # Only re-inject 1-4 terms internally during self-interaction. (Cross interactions have no covalent bonds).
...
log_info(f"Re-injected 1-4 Exceptions: VDW={vdw_14_count}, ELEC={elec_14_count}")
...
if is_self_interaction and any(e in final_components for e in ["bond", "angl", "dihe", "impr", "cmap"]):
    ...
    for f in base_system.getForces():
        ...
        if fname == "HarmonicBondForce" and "bond" in final_components:
            ...
            if added > 0:
                ...
                log_info(f" -> Dispatched {added} Harmonic Bonds (Group 3)")
...
FRAME_STEP = FRAME_SKIP + 1 if FRAME_SKIP > 0 else 1
...
while not SHUTDOWN_REQUESTED:
    ...
    if IS_DYNAMIC:
        idx_set1 = set(arr_sel1.tolist())
        idx_set2 = idx_set1 if is_self_interaction else (set(arr_sel2.tolist()) - idx_set1)

        diff1 = idx_set1 ^ prev_set1
        diff2 = idx_set2 ^ prev_set2

        for i in diff1:
            val = 1.0 if i in idx_set1 else 0.0
            if is_self_interaction:
                vdw_force.setParticleParameters(i, [*param_cache_vdw[i], val])
                elec_force.setParticleParameters(i, [*param_cache_elec[i], val])
            else:
                s2_val = 1.0 if i in idx_set2 else 0.0
        ...
        if not is_self_interaction:
            for i in diff2:
                if i in diff1: continue
                s2_val = 1.0 if i in idx_set2 else 0.0
                s1_val = 1.0 if i in idx_set1 else 0.0
    ...
    # Query Forces
    force_components = None  # [elec_force_components, vdw_force_components]
    ...
    # final data
    data_buffer.append((abs_f, n1 if UPDATE_SELECTION1 else None, n2 if UPDATE_SELECTION2 else None, erg_raw, force_mags, force_components))

    # --- PROGRESS TRACKER EXECUTION ---
    frames_processed += 1
    if frames_processed == next_report:
        t_now = time.time()
        fps_current = REPORT_INTERVAL / max(0.001, t_now - t_last_report)
