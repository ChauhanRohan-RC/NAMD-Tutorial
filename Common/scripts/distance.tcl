#######################################################################################
### Script to calculate DISTANCE B/W 2 ATOMS   from dcd trajectory				 	 ##
#######################################################################################

## USAGE ------------------------
# 1. Copy script to working dir
# 2. INPUT: Set input .psf and .dcd files
...
# ======================= INPUT ===========================
set in_psf_file "../common/dna.psf"
set in_dcd_file "dna_gbis_pcf.dcd"

set selection_atom1 "nucleic and resid 1 and name C5'"
set selection_atom2 "nucleic and resid 16 and name C3'"

# ======================= OUTPUT ===========================
set out_file_name "dist_vs_frame.dat";
set out_delimiter " \t ";
# -----------------------------------------------------
...
log "#=============  DISTANCE B/W ATOMS  ================="

# loading INPUT .psf and .dcd files
set mol_id [mol new $in_psf_file];	

puts "---------------------------------------------"
puts " -> Loading trajectory file : ${in_dcd_file}"
puts "---------------------------------------------"
mol addfile $in_dcd_file waitfor all molid $mol_id;
...
# =========================  MAIN  ==================================
puts "--------------------------"
log "#LOG: INPUT Structure File: ${in_psf_file} | Trajectory File: ${in_dcd_file}$"
log "#--------------------------"
